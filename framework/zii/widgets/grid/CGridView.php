...
OldYii::import('zii.widgets.CBaseListView');
OldYii::import('zii.widgets.grid.CDataColumn');
OldYii::import('zii.widgets.grid.CLinkColumn');
OldYii::import('zii.widgets.grid.CButtonColumn');
OldYii::import('zii.widgets.grid.CCheckBoxColumn');
...
class CGridView extends CBaseListView
{
	...
	public function init()
	{
		...
		if(empty($this->updateSelector))
			throw new CException(OldYii::t('zii','The property updateSelector should be defined.'));
		if(empty($this->filterSelector))
			throw new CException(OldYii::t('zii','The property filterSelector should be defined.'));

		if(!isset($this->htmlOptions['class']))
			$this->htmlOptions['class']='grid-view';

		if($this->baseScriptUrl===null)
			$this->baseScriptUrl=OldYii::app()->getAssetManager()->publish(OldYii::getPathOfAlias('zii.widgets.assets')).'/gridview';

		if($this->cssFile!==false)
		{
			if($this->cssFile===null)
				$this->cssFile=$this->baseScriptUrl.'/styles.css';
			OldYii::app()->getClientScript()->registerCssFile($this->cssFile);
		}
	...
	protected function initColumns()
	{
		...
		foreach($this->columns as $i=>$column)
		{
			...
			else
			{
				...
				$column=OldYii::createComponent($column, $this);
			}
			if(!$column->visible)
			{
	...
	protected function createDataColumn($text)
	{
		if(!preg_match('/^([\w\.]+)(:(\w*))?(:(.*))?$/',$text,$matches))
			throw new CException(OldYii::t('zii','The column must be specified in the format of "Name:Type:Label", where "Type" and "Label" are optional.'));
	...
	public function getFormatter()
	{
		if($this->_formatter===null)
			$this->_formatter=OldYii::app()->format;
