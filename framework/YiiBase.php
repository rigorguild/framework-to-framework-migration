...
/**
 * This constant defines how much call stack information (file name and line number) should be logged by OldYii::trace().
...
class YiiBase
{
	...
	public static function setApplication($app)
	{
		if(self::$_app===null || $app===null)
			...
			throw new CException(OldYii::t('yii','Yii application can only be created once.'));
	}
	...
	public static function import($alias,$forceInclude=false)
	{
		...
		if(($pos=strrpos($alias,'\\'))!==false) // a class name in PHP 5.3 namespace format
		{
			...
			if(($path=self::getPathOfAlias($namespace))!==false)
			{
				...
				if($forceInclude)
				{
					if(is_file($classFile))
						...
						throw new CException(OldYii::t('yii','Alias "{alias}" is invalid. Make sure it points to an existing PHP file and the file is readable.',array('{alias}'=>$alias)));
			...
			else
			{
				...
				if (class_exists($alias,true))
					...
					throw new CException(OldYii::t('yii','Alias "{alias}" is invalid. Make sure it points to an existing directory or file.',
		...
		if(($pos=strrpos($alias,'.'))===false)  // a simple class name
		{
			...
			if($forceInclude && (OldYii::autoload($alias,true) || class_exists($alias,true)))
				self::$_imports[$alias]=$alias;
			return $alias;
		}
					...
					foreach(self::$_includePaths as $path)
					{
						...
						if(is_file($classFile))
						{
							...
							if(YII_DEBUG && basename(realpath($classFile))!==$className.'.php')
								throw new CException(OldYii::t('yii','Class name "{class}" does not match class file "{file}".', array(
									'{class}'=>$className,
									'{file}'=>$classFile,
								)));
							break;
	 ...
	 * <pre>
	 * OldYii::beginProfile('block1');
	 * OldYii::beginProfile('block2');
	 * OldYii::endProfile('block2');
	 * OldYii::endProfile('block1');
	 * </pre>
	...
	{
		return OldYii::t('yii','Powered by {yii}.', array('{yii}'=>'<a href="http://www.yiiframework.com/" rel="external">Yii Framework</a>'));
	}
