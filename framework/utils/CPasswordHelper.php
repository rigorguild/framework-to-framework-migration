...
class CPasswordHelper
{
	...
	protected static function checkBlowfish()
	{
		if(!function_exists('crypt'))
			throw new CException(OldYii::t('yii','{class} requires the PHP crypt() function. This system does not have it.',
	...
	public static function hashPassword($password,$cost=13)
	{
		...
		if(!is_string($hash) || (function_exists('mb_strlen') ? mb_strlen($hash, '8bit') : strlen($hash))<32)
			throw new CException(OldYii::t('yii','Internal error while generating hash.'));
	...
	{
		if(!is_numeric($cost))
			throw new CException(OldYii::t('yii','{class}::$cost must be a number.',array('{class}'=>__CLASS__)));

		$cost=(int)$cost;
		if($cost<4 || $cost>31)
			throw new CException(OldYii::t('yii','{class}::$cost must be between 4 and 31.',array('{class}'=>__CLASS__)));

		if(($random=OldYii::app()->getSecurityManager()->generateRandomString(22,true))===false)
			if(($random=OldYii::app()->getSecurityManager()->generateRandomString(22,false))===false)
				throw new CException(OldYii::t('yii','Unable to generate random string.'));
