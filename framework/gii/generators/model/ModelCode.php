...
class ModelCode extends CCodeModel
{
	...
	{
		if(OldYii::app()->{$this->connectionId}===null)
			throw new CHttpException(500,'A valid database connection is required to run this generator.');
		$this->tablePrefix=OldYii::app()->{$this->connectionId}->tablePrefix;
		parent::init();
	}
	...
	public function prepare()
	{
		...
		{
			$tables=OldYii::app()->{$this->connectionId}->schema->getTables($schema);
			if($this->tablePrefix!='')
			{
		...
		foreach($tables as $table)
		{
			...
			$this->files[]=new CCodeFile(
				OldYii::getPathOfAlias($this->modelPath).'/'.$className.'.php',
	...
	{
		$connection=OldYii::app()->{$this->connectionId};
	...
	{
		if($addBrackets && OldYii::app()->{$this->connectionId}->tablePrefix=='')
			return $tableName;
		$prefix=$this->tablePrefix!='' ? $this->tablePrefix : OldYii::app()->{$this->connectionId}->tablePrefix;
		if($prefix!='')
		{
			if($addBrackets && OldYii::app()->{$this->connectionId}->tablePrefix!='')
			{
				$prefix=OldYii::app()->{$this->connectionId}->tablePrefix;
				$lb='{{';
				$rb='}}';
			}
			else
	...
	protected function generateRelations()
	{
		...
		$relations=array();
		foreach(OldYii::app()->{$this->connectionId}->schema->getTables($schemaName) as $table)
		{
			if($this->tablePrefix!='' && strpos($table->name,$this->tablePrefix)!==0)
				continue;
			$tableName=$table->name;
	...
	protected function generateRelationName($tableName, $fkName, $multiple)
	{
		...
		$rawName=$name;
		$table=OldYii::app()->{$this->connectionId}->schema->getTable($tableName);
		$i=0;
	...
	{
		if(OldYii::app()->hasComponent($this->connectionId)===false || !(OldYii::app()->getComponent($this->connectionId) instanceof CDbConnection))
