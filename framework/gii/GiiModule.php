...
OldYii::import('system.gii.CCodeGenerator');
OldYii::import('system.gii.CCodeModel');
OldYii::import('system.gii.CCodeFile');
OldYii::import('system.gii.CCodeForm');
...
class GiiModule extends CWebModule
{
	...
	{
		parent::init();
		OldYii::setPathOfAlias('gii',dirname(__FILE__));
		OldYii::app()->setComponents(array(
			'errorHandler'=>array(
				'class'=>'CErrorHandler',
			...
			'user'=>array(
				'class'=>'CWebUser',
				'stateKeyPrefix'=>'gii',
				'loginUrl'=>OldYii::app()->createUrl($this->getId().'/default/login'),
			),
			'widgetFactory' => array(
	...
	public function getAssetsUrl()
	{
		if($this->_assetsUrl===null)
			$this->_assetsUrl=OldYii::app()->getAssetManager()->publish(OldYii::getPathOfAlias('gii.assets'));
	...
	public function beforeControllerAction($controller, $action)
	{
		if(parent::beforeControllerAction($controller, $action))
		{
			...
			if(!$this->allowIp(OldYii::app()->request->userHostAddress) && $route!=='default/error')
			...
			if($this->password!==false && OldYii::app()->user->isGuest && !in_array($route,$publicPages))
				OldYii::app()->user->loginRequired();
			else
				return true;
		}
		return false;
	...
	protected function findGenerators()
	{
		...
		for($i=$n-1;$i>=0;--$i)
		{
			...
			$path=OldYii::getPathOfAlias($alias);
