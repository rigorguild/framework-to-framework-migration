...
class CCaptchaValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		{
			$message=$this->message!==null?$this->message:OldYii::t('yii','The verification code is incorrect.');
	...
	{
		if(($captcha=OldYii::app()->getController()->createAction($this->captchaAction))===null)
		{
			if(strpos($this->captchaAction,'/')!==false) // contains controller or module
			{
				if(($ca=OldYii::app()->createController($this->captchaAction))!==null)
				{
			...
			if($captcha===null)
				throw new CException(OldYii::t('yii','CCaptchaValidator.action "{id}" is invalid. Unable to find such an action in the current controller.',
