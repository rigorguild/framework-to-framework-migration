 ...
 * the directory that contains the extension class file. When using OldYii::t() to translate an extension message,
...
class CPhpMessageSource extends CMessageSource
{
	 ...
	 * When using OldYii::t() to translate an extension message, the category name should be
	...
	public function init()
	{
		...
		if($this->basePath===null)
			$this->basePath=OldYii::getPathOfAlias('application.messages');
	}
		...
		if(!isset($this->_files[$category][$language]))
		{
			if(($pos=strpos($category,'.'))!==false)
			{
				...
				if(isset($this->extensionPaths[$extensionClass]))
					$this->_files[$category][$language]=OldYii::getPathOfAlias($this->extensionPaths[$extensionClass]).DIRECTORY_SEPARATOR.$language.DIRECTORY_SEPARATOR.$extensionCategory.'.php';
				else
				{
	...
	protected function loadMessages($category,$language)
	{
		...
		if($this->cachingDuration>0 && $this->cacheID!==false && ($cache=OldYii::app()->getComponent($this->cacheID))!==null)
		{
