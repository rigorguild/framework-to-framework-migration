...
class CAssetManager extends CApplicationComponent
{
	...
	public function setBasePath($value)
	{
		if(($basePath=realpath($value))!==false && is_dir($basePath) && is_writable($basePath))
			...
			throw new CException(OldYii::t('yii','CAssetManager.basePath "{path}" is invalid. Please make sure the directory exists and is writable by the Web server process.',
				array('{path}'=>$value)));
	}
	...
	public function publish($path,$hashByName=false,$level=-1,$forceCopy=null)
	{
		...
		if($forceCopy && $this->linkAssets)
			throw new CException(OldYii::t('yii','The "forceCopy" and "linkAssets" cannot be both true.'));
		...
		throw new CException(OldYii::t('yii','The asset "{asset}" to be published does not exist.',
			array('{asset}'=>$path)));
	}
	...
	{
		return sprintf('%x',crc32($path.OldYii::getVersion()));
	}
