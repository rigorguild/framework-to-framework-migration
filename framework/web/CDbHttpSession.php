...
class CDbHttpSession extends CHttpSession
{
	...
	protected function getDbConnection()
	{
		...
		{
			if(($this->_db=OldYii::app()->getComponent($id)) instanceof CDbConnection)
				return $this->_db;
			else
				throw new CException(OldYii::t('yii','CDbHttpSession.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.',
					array('{id}'=>$id)));
		}
		else
		{
			$dbFile=OldYii::app()->getRuntimePath().DIRECTORY_SEPARATOR.'session-'.OldYii::getVersion().'.db';
