...
class CController extends CBaseController
{
	...
	{
		throw new CHttpException(400,OldYii::t('yii','Your request is invalid.'));
	}
	...
	{
		OldYii::app()->getClientScript()->render($output);
	...
	public function createAction($actionID)
	{
		...
		else
		{
			...
			if($action!==null && !method_exists($action,'run'))
				throw new CException(OldYii::t('yii', 'Action class {class} must implement the "run" method.', array('{class}'=>get_class($action))));
			return $action;
		}
	}
	...
	protected function createActionFromMap($actionMap,$actionID,$requestActionID,$config=array())
	{
		if(($pos=strpos($actionID,'.'))===false && isset($actionMap[$actionID]))
		{
			...
			return OldYii::createComponent(empty($config)?$baseConfig:array_merge($baseConfig,$config),$this,$requestActionID);
		}
		elseif($pos===false)
			return null;
		...
		elseif(is_array($provider) && isset($provider['class']))
		{
			...
			throw new CException(OldYii::t('yii','Object configuration must be an array containing a "class" element.'));

		$class=OldYii::import($providerType,true);
	...
	{
		throw new CHttpException(404,OldYii::t('yii','The system is unable to find the requested action "{action}".',
	...
	{
		if(($theme=OldYii::app()->getTheme())!==null && ($viewFile=$theme->getViewFile($this,$viewName))!==false)
			return $viewFile;
		$moduleViewPath=$basePath=OldYii::app()->getViewPath();
		if(($module=$this->getModule())!==null)
	...
	public function getLayoutFile($layoutName)
	{
		if($layoutName===false)
			return false;
		if(($theme=OldYii::app()->getTheme())!==null && ($layoutFile=$theme->getLayoutFile($this,$layoutName))!==false)
			return $layoutFile;
		...
		return $this->resolveViewFile($layoutName,$module->getLayoutPath(),OldYii::app()->getViewPath(),$module->getViewPath());
	}
	...
	public function resolveViewFile($viewName,$viewPath,$basePath,$moduleViewPath=null)
	{
		...
		if(($renderer=OldYii::app()->getViewRenderer())!==null)
		...
		elseif(strpos($viewName,'.'))
			$viewFile=OldYii::getPathOfAlias($viewName);
		else
			$viewFile=$viewPath.DIRECTORY_SEPARATOR.$viewName;

		if(is_file($viewFile.$extension))
			return OldYii::app()->findLocalizedFile($viewFile.$extension);
		elseif($extension!=='.php' && is_file($viewFile.'.php'))
			return OldYii::app()->findLocalizedFile($viewFile.'.php');
		else
			return false;
	}
	...
	public function forward($route,$exit=true)
	{
		...
		else
		{
			...
			OldYii::app()->runController($route);
		}
		if($exit)
			OldYii::app()->end();
	}
	...
	public function renderPartial($view,$data=null,$return=false,$processOutput=false)
	{
		if(($viewFile=$this->getViewFile($view))!==false)
		{
			...
			throw new CException(OldYii::t('yii','{controller} cannot find the requested view "{view}".',
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		if($route[0]!=='/' && ($module=$this->getModule())!==null)
			$route=$module->getId().'/'.$route;
		return OldYii::app()->createUrl(trim($route,'/'),$params,$ampersand);
	}
	...
	public function createAbsoluteUrl($route,$params=array(),$schema='',$ampersand='&')
	{
		...
		if(strpos($url,'http')===0)
			...
			return OldYii::app()->getRequest()->getHostInfo($schema).$url;
	}
	...
	public function getPageTitle()
	{
		...
		else
		{
			...
			if($this->getAction()!==null && strcasecmp($this->getAction()->getId(),$this->defaultAction))
				return $this->_pageTitle=OldYii::app()->name.' - '.ucfirst($this->getAction()->getId()).' '.$name;
			else
				return $this->_pageTitle=OldYii::app()->name.' - '.$name;
		}
	}
	...
	public function redirect($url,$terminate=true,$statusCode=302)
	{
		...
		OldYii::app()->getRequest()->redirect($url,$terminate,$statusCode);
	}
	...
	{
		$this->redirect(OldYii::app()->getRequest()->getUrl().$anchor,$terminate);
	}
	...
	{
		if(OldYii::app()->getRequest()->getIsPostRequest())
			$filterChain->run();
		else
			throw new CHttpException(400,OldYii::t('yii','Your request is invalid.'));
	}
	...
	{
		if(OldYii::app()->getRequest()->getIsAjaxRequest())
			$filterChain->run();
		else
			throw new CHttpException(400,OldYii::t('yii','Your request is invalid.'));
	}
	...
	protected function loadPageStates()
	{
		if(!empty($_POST[self::STATE_INPUT_NAME]))
		{
			if(($data=base64_decode($_POST[self::STATE_INPUT_NAME]))!==false)
			{
				...
				if(($data=OldYii::app()->getSecurityManager()->validateData($data))!==false)
	...
	{
		$data=OldYii::app()->getSecurityManager()->hashData(serialize($states));
		if(extension_loaded('zlib'))
