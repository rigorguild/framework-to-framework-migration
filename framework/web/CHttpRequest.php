...
class CHttpRequest extends CApplicationComponent
{
	...
	protected function normalizeRequest()
	{
		...
		if($this->enableCsrfValidation)
			OldYii::app()->attachEventHandler('onBeginRequest',array($this,'validateCsrfToken'));
	}
	...
	public function getScriptUrl()
	{
		if($this->_scriptUrl===null)
		{
			...
			elseif(isset($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'],$_SERVER['DOCUMENT_ROOT'])===0)
				...
				throw new CException(OldYii::t('yii','CHttpRequest is unable to determine the entry script URL.'));
		}
	...
	public function getPathInfo()
	{
		if($this->_pathInfo===null)
		{
			...
			elseif(strpos($_SERVER['PHP_SELF'],$scriptUrl)===0)
				...
				throw new CException(OldYii::t('yii','CHttpRequest is unable to determine the path info of the request.'));
	...
	public function getRequestUri()
	{
		if($this->_requestUri===null)
		{
			...
			elseif(isset($_SERVER['ORIG_PATH_INFO']))  // IIS 5.0 CGI
			{
				...
				throw new CException(OldYii::t('yii','CHttpRequest is unable to determine the request URI.'));
		}
	 ...
	 * <pre>
	 * <?php
	 *    OldYii::app()->request->xSendFile('/home/user/Pictures/picture1.jpg',array(
	 *        'saveName'=>'image1.jpg',
	 *        'mimeType'=>'image/jpeg',
	 *        'terminate'=>false,
	 *    ));
	...
	{
		$securityManager=OldYii::app()->getSecurityManager();
	...
	public function validateCsrfToken($event)
	{
			...
			$this->getIsDeleteRequest())
		{
			...
			if (!empty($maskedUserToken) && $cookies->contains($this->csrfTokenName))
			{
				...
				$valid = false;
			if (!$valid)
				throw new CHttpException(400,OldYii::t('yii','The CSRF token could not be verified.'));
		}
	}
...
class CCookieCollection extends CMap
{
	...
	protected function getCookies()
	{
		...
		{
			$sm=OldYii::app()->getSecurityManager();
	...
	public function add($name,$cookie)
	{
		if($cookie instanceof CHttpCookie)
		{
			...
			throw new CException(OldYii::t('yii','CHttpCookieCollection can only hold CHttpCookie objects.'));
	}
	 ...
	 * OldYii::app()->request->cookies['foo']=new CHttpCookie('cookie','value',$options);
	 * OldYii::app()->request->cookies->remove('cookie',$options);
	 * </pre>
	 *
	 * @param mixed $name Cookie name.
	...
	protected function addCookie($cookie)
	{
		$value=$cookie->value;
		if($this->_request->enableCookieValidation)
			$value=OldYii::app()->getSecurityManager()->hashData(serialize($value));
