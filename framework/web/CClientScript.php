...
class CClientScript extends CApplicationComponent
{
	...
	public function getCoreScriptUrl()
	{
		if($this->_baseUrl!==null)
			...
			return $this->_baseUrl=OldYii::app()->getAssetManager()->publish(YII_PATH.'/web/js/source');
	}
	...
	public function getPackageBaseUrl($name)
	{
		...
		if(isset($package['baseUrl']))
		{
			...
			if($baseUrl==='' || $baseUrl[0]!=='/' && strpos($baseUrl,'://')===false)
				$baseUrl=OldYii::app()->getRequest()->getBaseUrl().'/'.$baseUrl;
			$baseUrl=rtrim($baseUrl,'/');
		}
		elseif(isset($package['basePath']))
			$baseUrl=OldYii::app()->getAssetManager()->publish(OldYii::getPathOfAlias($package['basePath']));
		else
	...
	public function registerCoreScript($name)
	{
		...
		elseif(YII_DEBUG)
			...
			OldYii::log('There is no CClientScript package: '.$name,CLogger::LEVEL_WARNING,'system.web.CClientScript');
	...
	public function registerScript($id,$script,$position=null,array $htmlOptions=array())
	{
		...
		else
		{
			if($position==self::POS_LOAD || $position==self::POS_READY)
				throw new CException(OldYii::t('yii','Script HTML options are not allowed for "CClientScript::POS_LOAD" and "CClientScript::POS_READY".'));
	 ...
	 * This is a shortcut to OldYii::app()->controller->recordCachingAction.
	...
	{
		if(($controller=OldYii::app()->getController())!==null)
