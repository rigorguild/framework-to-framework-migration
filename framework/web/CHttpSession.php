...
class CHttpSession extends CApplicationComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	public function open()
	{
		...
		{
			$message=OldYii::t('yii','Failed to start session.');
			...
			OldYii::log($message, CLogger::LEVEL_WARNING, 'system.web.CHttpSession');
		}
	}
	...
	public function setSavePath($value)
	{
		if(is_dir($value))
			...
			throw new CException(OldYii::t('yii','CHttpSession.savePath "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setCookieMode($value)
	{
		...
		elseif($value==='only')
		{
			...
			throw new CException(OldYii::t('yii','CHttpSession.cookieMode can only be "none", "allow" or "only".'));
	}
	...
	public function setGCProbability($value)
	{
		if($value>=0 && $value<=100)
		{
			...
			throw new CException(OldYii::t('yii','CHttpSession.gcProbability "{value}" is invalid. It must be a float between 0 and 100.',
				array('{value}'=>$value)));
	}
