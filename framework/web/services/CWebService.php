...
class CWebService extends CComponent
{
	 ...
	 * to {@link OldYii::createComponent} to create the generator object. Default value is 'CWsdlGenerator'.
	 * @since 1.1.12
	 */
	...
	{
		$providerClass=is_object($this->provider) ? get_class($this->provider) : OldYii::import($this->provider,true);
		if($this->wsdlCacheDuration>0 && $this->cacheID!==false && ($cache=OldYii::app()->getComponent($this->cacheID))!==null)
		{
			$key='Yii.CWebService.'.$providerClass.$this->serviceUrl.$this->encoding;
			if(($wsdl=$cache->get($key))!==false)
				return $wsdl;
		}
		$generator=OldYii::createComponent($this->generatorConfig);
	...
	public function run()
	{
		...
		OldYii::app()->attachEventHandler('onError',array($this,'handleError'));
		try
		{
			if($this->persistence!==null)
				$server->setPersistence($this->persistence);
			if(is_string($this->provider))
				$provider=OldYii::createComponent($this->provider);
			else
		...
		catch(Exception $e)
		{
			if($e->getCode()!==self::SOAP_ERROR) // non-PHP error
			{
				...
				OldYii::log($e->__toString(),CLogger::LEVEL_ERROR,'application');
			}
			...
			OldYii::app()->onEndRequest(new CEvent($this));
	...
	protected function getOptions()
	{
		...
		{
			$className=OldYii::import($className,true);
			if(is_int($type))
				$type=$className;
