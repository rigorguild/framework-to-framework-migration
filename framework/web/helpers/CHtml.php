...
class CHtml
{
	...
	{
		return htmlspecialchars($text,ENT_QUOTES,OldYii::app()->charset);
	}
	...
	public static function encodeArray($data)
	{
		...
		{
			if(is_string($key))
				$key=htmlspecialchars($key,ENT_QUOTES,OldYii::app()->charset);
			if(is_string($value))
				$value=htmlspecialchars($value,ENT_QUOTES,OldYii::app()->charset);
	...
	public static function refresh($seconds,$url='')
	{
		...
		OldYii::app()->clientScript->registerMetaTag($content,null,'refresh');
	}
...
EOD;
	...
	{
		OldYii::app()->getClientScript()->registerCoreScript('jquery');
	...
	{
		return OldYii::app()->getAssetManager()->publish($path,$hashByName);
	}
	...
	public static function normalizeUrl($url)
	{
		if(is_array($url))
		{
			if(isset($url[0]))
			{
				if(($c=OldYii::app()->getController())!==null)
					$url=$c->createUrl($url[0],array_splice($url,1));
				else
					$url=OldYii::app()->createUrl($url[0],array_splice($url,1));
			}
			else
				$url='';
		}
		return $url==='' ? OldYii::app()->getRequest()->getUrl() : $url;
	}
	...
	public static function errorSummary($model,$header=null,$footer=null,$htmlOptions=array())
	{
		...
		{
			if($header===null)
				$header='<p>'.OldYii::t('yii','Please fix the following input errors:').'</p>';
			if(!isset($htmlOptions['class']))
	...
	public static function setModelNameConverter($converter)
	{
		...
		elseif($converter===null)
			...
			throw new CException(OldYii::t('yii','The $converter argument must be a valid callback or null.'));
	}
