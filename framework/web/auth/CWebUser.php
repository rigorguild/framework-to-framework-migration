...
class CWebUser extends CApplicationComponent implements IWebUser
{
		...
		if($this->beforeLogin($id,$states,false))
		{
			...
			if($duration>0)
			{
				if($this->allowAutoLogin)
					...
					throw new CException(OldYii::t('yii','{class}.allowAutoLogin must be set true in order to use cookie-based authentication.',
	...
	public function logout($destroySession=true)
	{
		if($this->beforeLogout())
		{
			if($this->allowAutoLogin)
			{
				OldYii::app()->getRequest()->getCookies()->remove($this->getStateKeyPrefix());
				if($this->identityCookie!==null)
				{
					$cookie=$this->createIdentityCookie($this->getStateKeyPrefix());
					$cookie->value=null;
					$cookie->expire=0;
					OldYii::app()->getRequest()->getCookies()->add($cookie->name,$cookie);
				}
			}
			if($destroySession)
				OldYii::app()->getSession()->destroy();
			else
				$this->clearStates();
			$this->_access=array();
			$this->afterLogout();
	...
	public function getReturnUrl($defaultUrl=null)
	{
		if($defaultUrl===null)
		{
			$defaultReturnUrl=OldYii::app()->getUrlManager()->showScriptName ? OldYii::app()->getRequest()->getScriptUrl() : OldYii::app()->getRequest()->getBaseUrl().'/';
		}
		else
		{
	...
	public function loginRequired()
	{
		...
		throw new CHttpException(403,OldYii::t('yii','Login Required'));
	}
	...
	protected function renewCookie()
	{
		...
		if($cookie && !empty($cookie->value) && ($data=OldYii::app()->getSecurityManager()->validateData($cookie->value))!==false)
		{
			$data=@unserialize($data);
			if(is_array($data) && isset($data[0],$data[1],$data[2],$data[3]))
			{
	...
	public function getStateKeyPrefix()
	{
		if($this->_keyPrefix!==null)
			...
			return $this->_keyPrefix=md5('Yii.'.get_class($this).'.'.OldYii::app()->getId());
	}
	...
	{
		OldYii::app()->getSession()->regenerateID(true);
		$this->setId($id);
		$this->setName($name);
	...
	public function checkAccess($operation,$params=array(),$allowCaching=true)
	{
		...
		$access=OldYii::app()->getAuthManager()->checkAccess($operation,$this->getId(),$params);
