...
class CUrlManager extends CApplicationComponent
{
	...
	protected function processRules()
	{
		if(empty($this->rules) || $this->getUrlFormat()===self::GET_FORMAT)
			return;
		if($this->cacheID!==false && ($cache=OldYii::app()->getComponent($this->cacheID))!==null)
		{
	...
	protected function createUrlRule($route,$pattern)
	{
		...
		{
			$urlRuleClass=OldYii::import($this->urlRuleClass,true);
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		{
			if(is_array($rule))
				$this->_rules[$i]=$rule=OldYii::createComponent($rule);
	...
	public function parseUrl($request)
	{
		if($this->getUrlFormat()===self::PATH_FORMAT)
		{
			...
			if($this->useStrictParsing)
				throw new CHttpException(404,OldYii::t('yii','Unable to resolve the request "{route}".',
	...
	public function getBaseUrl()
	{
		...
		{
			if($this->showScriptName)
				$this->_baseUrl=OldYii::app()->getRequest()->getScriptUrl();
			else
				$this->_baseUrl=OldYii::app()->getRequest()->getBaseUrl();
			return $this->_baseUrl;
		}
	}
	...
	public function setUrlFormat($value)
	{
		if($value===self::PATH_FORMAT || $value===self::GET_FORMAT)
			...
			throw new CException(OldYii::t('yii','CUrlManager.UrlFormat must be either "path" or "get".'));
	}
}
...
class CUrlRule extends CBaseUrlRule
{
	...
	public function __construct($route,$pattern)
	{
		...
		if(YII_DEBUG && @preg_match($this->pattern,'test')===false)
			throw new CException(OldYii::t('yii','The URL pattern "{pattern}" for route "{route}" is not a valid regular expression.',
	...
	public function createUrl($manager,$route,$params,$ampersand)
	{
		...
		if($this->hasHostInfo)
		{
			$hostInfo=OldYii::app()->getRequest()->getHostInfo();
