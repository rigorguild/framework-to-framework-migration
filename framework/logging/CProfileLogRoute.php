...
class CProfileLogRoute extends CWebLogRoute
{
	...
	public function setReport($value)
	{
		if($value==='summary' || $value==='callstack')
			...
			throw new CException(OldYii::t('yii','CProfileLogRoute.report "{report}" is invalid. Valid values include "summary" and "callstack".',
				array('{report}'=>$value)));
	}
	 ...
	 * @throws CException if OldYii::beginProfile() and OldYii::endProfile() are not matching
	 */
		...
		foreach($logs as $log)
		{
			...
			elseif(!strncasecmp($message,'end:',4))
			{
				...
				if(($last=array_pop($stack))!==null && $last[0]===$token)
				{
					...
					throw new CException(OldYii::t('yii','CProfileLogRoute found a mismatching code block "{token}". Make sure the calls to OldYii::beginProfile() and OldYii::endProfile() be properly nested.',
						array('{token}'=>$token)));
			}
		}
