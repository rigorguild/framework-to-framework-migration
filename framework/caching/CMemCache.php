	...
	public function getMemCache()
	{
		...
		else
		{
			...
			if(!extension_loaded($extension))
				throw new CException(OldYii::t('yii',"CMemCache requires PHP {extension} extension to be loaded.",
	...
	public function __construct($config)
	{
		if(is_array($config))
		{
			foreach($config as $key=>$value)
				$this->$key=$value;
			if($this->host===null)
				throw new CException(OldYii::t('yii','CMemCache server configuration must have "host" value.'));
		}
		else
			throw new CException(OldYii::t('yii','CMemCache server configuration must be an array.'));
	}
}
