...
abstract class CCache extends CApplicationComponent implements ICache, ArrayAccess
{
	...
	public function init()
	{
		...
		if($this->keyPrefix===null)
			$this->keyPrefix=OldYii::app()->getId();
	}
	...
	public function get($id)
	{
		...
		{
			OldYii::trace('Serving "'.$id.'" from cache','system.caching.'.get_class($this));
			return $value[0];
		}
		else
			return false;
	...
	{
		OldYii::trace('Saving "'.$id.'" to cache','system.caching.'.get_class($this));
	...
	{
		OldYii::trace('Adding "'.$id.'" to cache','system.caching.'.get_class($this));
	...
	{
		OldYii::trace('Deleting "'.$id.'" from cache','system.caching.'.get_class($this));
	...
	{
		OldYii::trace('Flushing cache','system.caching.'.get_class($this));
	...
	{
		throw new CException(OldYii::t('yii','{className} does not support get() functionality.',
	...
	{
		throw new CException(OldYii::t('yii','{className} does not support set() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(OldYii::t('yii','{className} does not support add() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(OldYii::t('yii','{className} does not support delete() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(OldYii::t('yii','{className} does not support flushValues() functionality.',
			array('{className}'=>get_class($this))));
	}
