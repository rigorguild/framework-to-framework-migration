...
abstract class CDbSchema extends CComponent
{
	...
	public function getTable($name,$refresh=false)
	{
		...
		else
		{
			...
			if(!isset($this->_cacheExclude[$name]) && ($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=OldYii::app()->getComponent($this->_connection->schemaCacheID))!==null)
			{
	...
	{
		if(($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=OldYii::app()->getComponent($this->_connection->schemaCacheID))!==null)
		{
	...
	{
		throw new CDbException(OldYii::t('yii','{class} does not support fetching all table names.',
