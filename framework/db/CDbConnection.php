...
class CDbConnection extends CApplicationComponent
{
	...
	protected function open()
	{
		if($this->_pdo===null)
		{
			...
			try
			{
				OldYii::trace('Opening DB connection','system.db.CDbConnection');
			...
			catch(PDOException $e)
			{
				...
				{
					OldYii::log($e->getMessage(),CLogger::LEVEL_ERROR,'exception.CDbException');
	...
	{
		OldYii::trace('Closing DB connection','system.db.CDbConnection');
		$this->_pdo=null;
		$this->_active=false;
		$this->_schema=null;
	}
	...
	protected function createPdoInstance()
	{
		...
		if(!class_exists($pdoClass))
			throw new CDbException(OldYii::t('yii','CDbConnection is unable to find PDO class "{className}". Make sure PDO is installed correctly.',
				array('{className}'=>$pdoClass)));

		@$instance=new $pdoClass($this->connectionString,$this->username,$this->password,$this->_attributes);

		if(!$instance)
			throw new CDbException(OldYii::t('yii','CDbConnection failed to open the DB connection.'));
	...
	{
		OldYii::trace('Starting transaction','system.db.CDbConnection');
		$this->setActive(true);
	...
	public function getSchema()
	{
		...
		else
		{
			...
			if(isset($this->driverMap[$driver]))
				return $this->_schema=OldYii::createComponent($this->driverMap[$driver], $this);
			else
				throw new CDbException(OldYii::t('yii','CDbConnection does not support reading schema for {driver} database.',
					array('{driver}'=>$driver)));
		}
	}
