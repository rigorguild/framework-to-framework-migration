 ...
 * <pre>
 * $user = OldYii::app()->db->createCommand()
...
class CDbCommand extends CComponent
{
	...
	public function prepare()
	{
		if($this->_statement==null)
		{
			...
			{
				OldYii::log('Error in preparing SQL: '.$this->getText(),CLogger::LEVEL_ERROR,'system.db.CDbCommand');
				$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
				throw new CDbException(OldYii::t('yii','CDbCommand failed to prepare the SQL statement: {error}',
	...
	public function execute($params=array())
	{
		if($this->_connection->enableParamLogging && ($pars=array_merge($this->_paramLog,$params))!==array())
		{
			...
			$par='';
		OldYii::trace('Executing SQL: '.$this->getText().$par,'system.db.CDbCommand');
		try
		{
			if($this->_connection->enableProfiling)
				OldYii::beginProfile('system.db.CDbCommand.execute('.$this->getText().$par.')','system.db.CDbCommand.execute');
			...
			if($this->_connection->enableProfiling)
				OldYii::endProfile('system.db.CDbCommand.execute('.$this->getText().$par.')','system.db.CDbCommand.execute');
		...
		catch(Exception $e)
		{
			...
			OldYii::log(OldYii::t('yii','CDbCommand::execute() failed: {error}. The SQL statement executed was: {sql}.',
				array('{error}'=>$message, '{sql}'=>$this->getText().$par)),CLogger::LEVEL_ERROR,'system.db.CDbCommand');

			if(YII_DEBUG)
				$message.='. The SQL statement executed was: '.$this->getText().$par;

			throw new CDbException(OldYii::t('yii','CDbCommand failed to execute the SQL statement: {error}',
	...
	private function queryInternal($method,$mode,$params=array())
	{
		...
		OldYii::trace('Querying SQL: '.$this->getText().$par,'system.db.CDbCommand');

		if($this->_connection->queryCachingCount>0 && $method!==''
				&& $this->_connection->queryCachingDuration>0
				&& $this->_connection->queryCacheID!==false
				&& ($cache=OldYii::app()->getComponent($this->_connection->queryCacheID))!==null)
		{
			...
			{
				OldYii::trace('Query result found in cache','system.db.CDbCommand');
				return $result[0];
			}
		}
		...
		{
			if($this->_connection->enableProfiling)
				OldYii::beginProfile('system.db.CDbCommand.query('.$this->getText().$par.')','system.db.CDbCommand.query');
			...
			if($this->_connection->enableProfiling)
				OldYii::endProfile('system.db.CDbCommand.query('.$this->getText().$par.')','system.db.CDbCommand.query');
		...
		catch(Exception $e)
		{
			...
			$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
			$message=$e->getMessage();
			OldYii::log(OldYii::t('yii','CDbCommand::{method}() failed: {error}. The SQL statement executed was: {sql}.',
	...
	private function processConditions($conditions)
	{
		...
		throw new CDbException(OldYii::t('yii', 'Unknown operator "{operator}".', array('{operator}'=>$operator)));
	}
