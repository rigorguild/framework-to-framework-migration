...
abstract class CApplication extends CModule
{
	...
	public function __construct($config=null)
	{
		...
		OldYii::setPathOfAlias('application',$this->getBasePath());
		OldYii::setPathOfAlias('webroot',dirname($_SERVER['SCRIPT_FILENAME']));
		if(isset($config['extensionPath']))
		{
			...
			OldYii::setPathOfAlias('ext',$this->getBasePath().DIRECTORY_SEPARATOR.'extensions');
	...
	public function setBasePath($path)
	{
		if(($this->_basePath=realpath($path))===false || !is_dir($this->_basePath))
			throw new CException(OldYii::t('yii','Application base path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setRuntimePath($path)
	{
		if(($runtimePath=realpath($path))===false || !is_dir($runtimePath) || !is_writable($runtimePath))
			throw new CException(OldYii::t('yii','Application runtime path "{path}" is not valid. Please make sure it is a directory writable by the Web server process.',
				array('{path}'=>$path)));
	...
	{
		return OldYii::getPathOfAlias('ext');
	}
	...
	public function setExtensionPath($path)
	{
		if(($extensionPath=realpath($path))===false || !is_dir($extensionPath))
			throw new CException(OldYii::t('yii','Extension path "{path}" does not exist.',
				array('{path}'=>$path)));
		OldYii::setPathOfAlias('ext',$extensionPath);
	}
	...
	public function getLocaleDataPath()
	{
		...
		if(empty($vars['dataPath']))
			return OldYii::getPathOfAlias('system.i18n.data');
	...
	public function handleException($exception)
	{
		...
		$message.="\n---";
		OldYii::log($message,CLogger::LEVEL_ERROR,$category);
	...
	public function handleError($code,$message,$file,$line)
	{
		if($code & error_reporting())
		{
			...
			OldYii::log($log,CLogger::LEVEL_ERROR,'php');

			try
			{
				OldYii::import('CErrorEvent',true);
