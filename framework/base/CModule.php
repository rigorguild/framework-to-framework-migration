...
abstract class CModule extends CComponent
{
	...
	public function __construct($id,$parent,$config=null)
	{
		...
		OldYii::setPathOfAlias($id,$this->getBasePath());
	...
	public function setBasePath($path)
	{
		if(($this->_basePath=realpath($path))===false || !is_dir($this->_basePath))
			throw new CException(OldYii::t('yii','Base path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setModulePath($value)
	{
		if(($this->_modulePath=realpath($value))===false || !is_dir($this->_modulePath))
			throw new CException(OldYii::t('yii','The module path "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setAliases($mappings)
	{
		...
		{
			if(($path=OldYii::getPathOfAlias($alias))!==false)
				OldYii::setPathOfAlias($name,$path);
			else
				OldYii::setPathOfAlias($name,$alias);
		}
	}
	...
	public function getModule($id)
	{
		...
		elseif(isset($this->_moduleConfig[$id]))
		{
			...
			{
				OldYii::trace("Loading \"$id\" module",'system.base.CModule');
				$class=$config['class'];
				unset($config['class'], $config['enabled']);
				if($this===OldYii::app())
					$module=OldYii::createComponent($class,$id,null,$config);
				else
					$module=OldYii::createComponent($class,$this->getId().'/'.$id,$this,$config);
	...
	public function setModules($modules,$merge=true)
	{
		foreach($modules as $id=>$module)
		{
			...
			else
			{
				...
				{
					if (OldYii::getPathOfAlias($id)===false)
						OldYii::setPathOfAlias($id,$this->getModulePath().DIRECTORY_SEPARATOR.$id);
	...
	public function getComponent($id,$createIfNull=true)
	{
		...
		elseif(isset($this->_componentConfig[$id]) && $createIfNull)
		{
			...
			if(!isset($config['enabled']) || $config['enabled'])
			{
				OldYii::trace("Loading \"$id\" application component",'system.CModule');
				unset($config['enabled']);
				$component=OldYii::createComponent($config);
				$component->init();
