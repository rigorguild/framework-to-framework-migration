...
class CErrorHandler extends CApplicationComponent
{
	 ...
	 * Inside the action, it can retrieve the error information by OldYii::app()->errorHandler->error.
	...
	protected function handleException($exception)
	{
		...
		if($app instanceof CWebApplication)
		{
			...
			if(!headers_sent())
			{
				$httpVersion=OldYii::app()->request->getHttpVersion();
	...
	protected function renderException()
	{
		...
		else
		{
			if($this->isAjaxRequest())
				OldYii::app()->displayException($exception);
			else
	...
	protected function renderError()
	{
		if($this->errorAction!==null)
			OldYii::app()->runController($this->errorAction);
		else
		{
			$data=$this->getError();
			if($this->isAjaxRequest())
				OldYii::app()->displayError($data['code'],$data['message'],$data['file'],$data['line']);
			elseif(YII_DEBUG)
	...
	{
		$viewPaths=array(
			OldYii::app()->getTheme()===null ? null :  OldYii::app()->getTheme()->getSystemViewPath(),
			OldYii::app() instanceof CWebApplication ? OldYii::app()->getSystemViewPath() : null,
	...
	protected function getVersionInfo()
	{
		if(YII_DEBUG)
		{
			$version='<a href="http://www.yiiframework.com/">Yii Framework</a>/'.OldYii::getVersion();
