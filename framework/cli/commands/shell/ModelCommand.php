	...
	public function run($args)
	{
		...
		if(empty($matches[1]))  // without regular expression
		{
			...
			if(($pos=strrpos($className,'.'))===false)
				$basePath=OldYii::getPathOfAlias('application.models');
			else
			{
				$basePath=OldYii::getPathOfAlias(substr($className,0,$pos));
		...
		else  // with regular expression
		{
			...
			if($pos>0)  // only regexp is given
				$basePath=OldYii::getPathOfAlias(rtrim(substr($className,0,$pos),'.'));
			else
				$basePath=OldYii::getPathOfAlias('application.models');
		...
		$fixturePath=$this->fixturePath===null?OldYii::getPathOfAlias('application.tests.fixtures'):$this->fixturePath;
		$unitTestPath=$this->unitTestPath===null?OldYii::getPathOfAlias('application.tests.unit'):$this->unitTestPath;
...
EOD;
	...
	{
		$tablePrefix=OldYii::app()->getDb()->tablePrefix;
