...
EOD;
	...
	public function run($args)
	{
		...
		else
		{
			...
			if(($m=OldYii::app()->getModule($id))!==null)
				$module=$m;
		}
		$modelClass=OldYii::import($modelClass);
		...
		if(isset($args[1]))
		{
			...
			else
			{
				...
				if(($m=OldYii::app()->getModule($first))!==null)
					$module=$m;
				else
				{
		...
		$functionalTestPath=$this->functionalTestPath===null?OldYii::getPathOfAlias('application.tests.functional'):$this->functionalTestPath;
	...
	public function generateController($source,$params)
	{
		...
		if($id===null)
			throw new ShellException(OldYii::t('yii','Error: Table "{table}" does not have a primary key.',array('{table}'=>$model->tableName())));
		elseif(is_array($id))
			throw new ShellException(OldYii::t('yii','Error: Table "{table}" has a composite primary key which is not supported by crud command.',array('{table}'=>$model->tableName())));
